...
{ U32 span_filter_s=(msgType==WARNINGMSG_VIBRATE)?60:SPAN_MSG_PUSH_FILTER_S;//震动消息过滤时间设定为60妙
  ...
  MYSQL_RES *res=db_queryf("select mc_users.id,mc_users.username,mc_users.session,mc_devices.state from `mc_users`,`mc_devices` where mc_devices.id=%u and mc_devices.username=mc_users.username",deviceID);
  if(res){ 
    U32 binded_userid;
    ...
    if(row && (binded_userid=atoi(row[0]))){
      ...
      if((msgType==WARNINGMSG_VIBRATE && atoi(row[3])==DEVSTATE_SLEEP) || msgType==WARNINGMSG_LOWPOWER || msgType==WARNINGMSG_FLOWDEPLETE) //强制发送短信(不管用户是否允许接收消息推送)
      {	//为降低成本，非休眠状态下的震动不发短信。
      	sms_send(binded_phone,msgContent);//will consume much time
      }
    }  
